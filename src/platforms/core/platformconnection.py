...
from platforms.util.geometry import prism
...
S_NOT_A_PLATFORM = 'This is not a platforms!'
...
def player_action(player, select, inspect):
    ...
    elif state and 'platforms' in state.name:
        platform = protocol.get_platform(x, y, z)
        if select and state.name == 'select platforms':
            if platform:
