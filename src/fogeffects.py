...
def lightning(connection, player=None):
    ...
    effects = [
        ...
        FogGradient(protocol, 4.0, (0, 0, 0), protocol.get_real_fog_color, ease_out)
    ]
...
def fade(connection, r, g, b, time = None):
    ...
    fade_time = time * 0.25
    effects = [
        FogGradient(protocol, fade_time, protocol.get_real_fog_color, color, ease_in),
        FogHold(protocol, time, color),
        FogGradient(protocol, fade_time, color, protocol.get_real_fog_color, ease_out)
    ]
    protocol.set_fog_effects(effects)
